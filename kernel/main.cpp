...
{
  const auto color = visible ? ToColor(0x000000) : ToColor(0xffffff);
    ...
    const acpi::RSDP &acpi_table)
{
  ...
  while (true)
  {
    ...
    case Message::kKeyPush:
      if (auto active_layer_id = active_layer->GetActive(); active_layer_id == text_window_layer_id)
      {
        InputTextWindow(msg->arg.keyboard.ascii);
      }
      else if (active_layer_id == task_b_window_layer_id)
      {
      ...
      {
        __asm__("cli");
        auto task_it = layer_task_map->find(active_layer_id);
        __asm__("sti");
